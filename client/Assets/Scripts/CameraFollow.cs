...
public class CameraFollow : MonoBehaviour
{
    ...
    public Transform mapBounds; // 벽 4개를 묶어둔 부모 오브젝트
    ...
    {
        // 1. MapBounds 안에 있는 4개의 벽(Collider)을 모두 찾아서 하나의 거대한 사각형(Bounds)으로 합칩니다.
        ...
        // 2. 현재 카메라 화면의 절반 높이와 너비를 계산합니다.
        float camHeight = cam.orthographicSize;
        float camWidth = camHeight * cam.aspect;

        // 3. 전체 맵 크기에서 카메라 화면의 절반 크기만큼을 빼서, 카메라 중심점의 한계 좌표를 만듭니다.
    ...
    private void LateUpdate()
    {
        ...
        // mapBounds가 할당되어 있다면 자동 계산된 구역 안에 카메라를 가둡니다.
        if (mapBounds != null)
        {
